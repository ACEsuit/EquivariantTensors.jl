...
using EquivariantTensors.SO3: Ctran, gram
...
for L = 0:Lmax 
   ...
   local X, Q, refl, B1, B2, B3, B4, B5, B6, B7, B8
   local rk, rk_r, ntest
   ...
   for (itest, (nn, ll)) in enumerate(nnll_list)
      ...
      Ure, Mll = EquivariantTensors.O3.coupling_coeffs(L, ll, nn; PI = false) # cSH based re_basis
      Ure_r, Mll_r = EquivariantTensors.O3.coupling_coeffs(L, ll, nn; PI = false, basis = real) # rSH based re_basis
      Urpe, Mll_rpe = EquivariantTensors.O3.coupling_coeffs(L, ll, nn) # cSH based rpe_basis
      Urpe_r, Mll_r_rpe = EquivariantTensors.O3.coupling_coeffs(L, ll, nn; basis = real) # rSH based rpe_basis
      ...
      refl = rand([-1,1])
      Q = refl .* RotZYZ(θ...)
      D = refl^L .* transpose(WignerD.wignerD(L, θ...))
   ...
   for (itest, ll) in enumerate(ll_list)
      ...
      Ure, Mll = EquivariantTensors.O3.coupling_coeffs(L, ll; PI = false) # cSH based re_basis
      Ure_r, Mll_r = EquivariantTensors.O3.coupling_coeffs(L, ll; PI = false, basis = :SpheriCart) # rSH based re_basis
      Urpe, Mll = EquivariantTensors.O3.coupling_coeffs(L, ll) # cSH based rpe_basis
      Urpe_r, Mll_r = EquivariantTensors.O3.coupling_coeffs(L, ll; basis = :SpheriCart) # rSH based rpe_basis
