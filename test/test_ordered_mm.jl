...
for i = 1:length(nnll_list_short)
   ...
   for Ltot in (iseven(sum(ll)) ? (0:2:4) : (1:2:3))
      ...
      if rank(gram(C_rpe)) > 0
         ...
         print_tf(@test rank(gram(C_rpe_recursive)) == rank(gram(C_rpe)) == rank(gram(C_rpe_ref)))
