...
function submset(lmax, lth)
    ...
    if lth == 1
        ...
        mset = Vector{Int64}[]
        for t in tmp
            set = [ push!(copy(t), l) for l in t[end]:lmax]
            push!(mset, set...)
        end
    end
    return mset
...
function signed_mmset(mm::Vector{T}, prune = true) where {T}
    N = length(mm); len = 2^N
...
#
function m_generate(nn::NTuple{N,T}, ll::NTuple{N,T}, L::T, k::T, basis) where {N, T} 
    @assert abs(k) ≤ L
    S = Sn(nn, ll)
    Nperm = length(S) - 1
    ordered_mset = [ submset(ll[S[i]], S[i+1]-S[i]) for i = 1:Nperm ]
    MM = Vector{Vector{T}}[]
    Total_length = 0
    MORD = Iterators.product(ordered_mset...)
    for m_ord in MORD 
        ...
        if m_filter(m_ord_reshape, k, basis)
            class_m = vcat( Iterators.product( 
                            [ multiset_permutations(m_ord[i], S[i+1]-S[i]) 
                              for i in 1:Nperm]...)... )
            push!(MM, [vcat(mm1...) for mm1 in class_m])
            Total_length += length(class_m)
        end
    end
    return MM, Total_length
    # return [ MM[i][t], N)  
    #          for i = 1:length(MM) ], Total_length
end
...
#                  intersect
m_generate(n,l,L, basis ) = 
        union([m_generate(n,l,L,k, basis)[1] for k in -L:L]...), 
        sum(length.(union([m_generate(n,l,L,k, basis)[1] for k in -L:L]...)))
...
function lexi_ord(nn::NTuple{N, T}, ll::NTuple{N, T}) where {N, T}
   bb = [ (ll[i], nn[i]) for i = 1:N ]
   p = sortperm(bb)
   # TODO: can skip this step I think? 
   bb_sorted = bb[p]
   return ntuple(i -> bb_sorted[i][2], N), 
          ntuple(i -> bb_sorted[i][1], N), 
          invperm(p)
end
    ...
    if r == 0 
        ...
        for i in 1:r
            ...
            for (j, m_class) in enumerate(MMmat)
                for mm in m_class
                    c += 1
                    cg_coef = GCG(ll, mm, Lset[i], basis; vectorize = (L!=0))
                    FMatrix[i,j] += cg_coef
                    UMatrix[i,c]  = cg_coef
                end
            end
